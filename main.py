...
def get_turtle(turtle_id):
    ...
    if "markerGalleryPhoto" in turtle_data:
        ...
        return (
            base
            + turtle_data["markerGalleryPhoto"]["imageUrl"]
            + f"?accessToken={token}"
            + f"&turtle_id={turtle_id}"
            + f"&type={type}"
        )
...
def scrape_turtlesat(type):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        scraped_urls = list(
            tqdm(
                executor.map(get_turtle, marker_ids),
                total=len(marker_ids),
                desc=f"Scraping Images type : {type}",
            )
        )
...
def download_images(img_urls):
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        list(
            tqdm(
                executor.map(get_image, img_urls),
                total=len(img_urls),
